...
| MOVX / MOVY double memory    | BASIC       | X/Y bus access implemented;        |
|                              |             | addressing modes simplified        |
| DSP operations (PADD etc.)   | DONE        | 20+ operations implemented:        |
|                              |             | PADD, PSUB, PMULS+PCLR/PSUB/PADD,  |
|                              |             | PCLR, PCOPY, PNEG, PABS, PINC,    |
|                              |             | PDEC, PAND, POR, PXOR, PCMP,      |
|                              |             | PSHL, PSHA, PADDC, PSTS, PLDS     |
| DCT / DCF variants           | DONE        | Check DSR.DC bit; skip if          |
|                              |             | condition not met                  |
| Repeat loop (LDS RC, LDRS,   | DONE        | RS/RE/RC registers added to CPU;   |
|   LDRE, LDRC)                |             | zero-overhead loop in CPU.step()   |
| Guard bit (A0G/A1G) updates  | DONE        | Updated on MOVS.W loads to A0/A1   |
|                              |             | and on DSP ops writing to A0/A1    |
| SR.T updates from PCMP       | DONE        | PCMP sets SR.T based on Sx > Sy    |

### Test Coverage

44 tests in `test_dsp_movs.py` + 31 tests in `test_dsp_ops.py` = 75 DSP tests:
- MOVS encoding / dispatch (6 tests)
- MOVS direct load/store modes 4-7 (10 tests)
- MOVS pre-decrement modes 8-11 (4 tests)
- MOVS post-increment modes 12-15 (6 tests)
- MOVS indexed modes 0-3 (4 tests)
- MOVS all 4 As registers (1 test)
- MOVS all 10 Ds registers, word load (1 test)
- MOVS all 10 Ds registers, long load (1 test)
- MOVS SigmaDelta2 user example (2 tests: load + store sequences)
- DSP operation stub (3 tests)
- MOVX/MOVY stub (3 tests)
- SR.DSP gate (3 tests)
- DSP operations: PCLR, PADD, PSUB, PCOPY, PCMP, PNEG, PABS, PAND/POR/PXOR,
  PSHL, PSTS/PLDS, DCT/DCF variants (20+ tests)
- Repeat loop: LDS RS/RE/RC, STS RS/RE/RC, loop branch-back, loop
  termination, DSR.DC updates (5 tests)

All 75 DSP tests pass.  All 115 total tests pass (14 CPU fixes + 7 TMU +
12 RTC + 7 UBC + 7 DMA + 44 MOVS + 31 DSP ops = 115, counting TMU as 7).
